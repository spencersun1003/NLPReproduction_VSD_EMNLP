...
class Trainer(TrainerBase):
    ...
    def evaluate(self, loader, dump_path=None, one_step_dec=False):
        ...
        if dump_path is None:
            ...
            # print("-----------Truth--------------------")
            # print(targets[0])
            # predictions=remove_adj(predictions)
            # targets=remove_adj(targets)
...
def remove_adj(pred_list):
    new_pred_list=[]

    for sen in pred_list:
        islist=False
        ...
        if islist:
            new_pred_list.append([' '.join(new_words)])
        else:
            new_pred_list.append(' '.join(new_words))
...
def main_worker(gpu, args):
    ...
    if args.predict_only:
        ...
        if args.evaluate_only:
            eva=trainer.evaluate(val_loader)
            print(eva)
        else:
